...
Content:
net.ipv4.ip_forward = 0 (here we want 0 but it's safer to be explicicit and make it permanent)
...
This is maybe a problem because it breaks isolation with the internet.
...
- LAN NIC has no gateway (it means no default route)
- Dual NIC = IP forwarding OFF (never a router)
