# Learning Log — Cloudflare Pages & Workers (First Project)
...
- **Routing**: File-based (e.g., `functions/api/render.js` → `/api/render`)
...
├── public/
│   └── index.html           # Frontend (file list)
├── functions/
│   └── api/
│       └── render.js        # Backend Worker (Markdown converter)
├── package.json
├── node_modules/
...
Cloudflare serves public/index.html (file list)
    ↓
User clicks on "2026-01-16-networking.md"
    ↓
Request: /api/render?file=2026/2026-01-16-networking.md
    ↓
Worker (functions/api/render.js) executes:
  1. Fetches https://raw.githubusercontent.com/.../2026-01-16-networking.md
...
├── Local server (PostgreSQL + FastAPI)
└── Sync to cloud ↕️

Cloudflare (Backend)
├── Workers: Sync API, validation, user management
├── R2: Product images, backups, reports
├── D1: Shared product catalog, multi-restaurant config
...
✅ Wrangler CLI (professional tool)  
✅ Serverless backend concepts  
✅ JavaScript Workers (similar to FastAPI but JS)  
...
```
md-viewer/
├── public/index.html        (68 lines) - File list UI
├── functions/api/render.js  (87 lines) - Markdown converter Worker
├── package.json             (12 lines) - Dependencies
└── wrangler.toml            (3 lines)  - Configuration
```

**Total code**: ~170 lines for a fully functional, globally deployed app! 🚀
...
**End of log** — Project successfully deployed! 🎉
